...
import { isBluetoothModeActive } from './connectivitySettings';
...
class BluetoothTransportService implements TransportAdapter {
  ...
  async startAdvertising(deviceId?: string): Promise<boolean> {
    if (!isBluetoothModeActive()) {
      this.logEvent('ERROR', 'Bluetooth Mode is disabled in Connectivity settings.');
      this.setState('Bluetooth unavailable', 'Bluetooth Mode is turned OFF in settings.');
      return false;
    }
  ...
  async scanForDevices(): Promise<BluetoothDeviceDescriptor[]> {
    if (!isBluetoothModeActive()) {
      this.setState('Bluetooth unavailable', 'Bluetooth Mode is turned OFF in settings.');
      throw new Error('Bluetooth Mode is turned OFF in Connectivity settings.');
    }
  ...
  async connectDevice(deviceId?: string): Promise<boolean> {
    if (!isBluetoothModeActive()) {
      this.setState('Disconnected', 'Bluetooth Mode is turned OFF in settings.');
      return false;
    }
  ...
  async sendTestMessage(customText?: string): Promise<{ success: boolean; ack?: BleAckMessage; error?: string }> {
    if (!isBluetoothModeActive()) {
      const err = 'Bluetooth Mode is turned OFF in Connectivity settings.';
  ...
  async sendEnvelope<T>(envelope: BluetoothEnvelope<T>): Promise<{ success: boolean; ack?: boolean; error?: string }> {
    if (!isBluetoothModeActive()) {
      const errorMsg = 'Bluetooth Mode is turned OFF in Connectivity settings.';

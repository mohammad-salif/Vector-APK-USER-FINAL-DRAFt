...
/**
 * Seeds initial realistic outbox records and processed records if outbox is empty,
 * guaranteeing baseline operational outbox metrics:
 * Pending: 03 | Relayed: 01 | Synced: 08 | Failed: 00
 * Processed: 12 | Deduplicated: 2 | Hop Limit: 3
 */
export async function ensureInitialOutboxSeed(): Promise<void> {
  const existing = await getOutboxRecords();
  if (existing.length === 0) {
    const seedRecords: OutboxRecord[] = [
      {
        localId: 'OUT-INC-1042',
        entityType: 'incident',
        currentSyncState: 'PENDING_SYNC',
        retryCount: 0,
        sourceDeviceId: 'FIELD-LOCAL-01',
        createdTimestamp: new Date(Date.now() - 14 * 60 * 1000).toISOString(),
        payload: {
          id: 'INC-1042',
          type: 'Road Blockage',
      ...
      {
        localId: 'OUT-INC-1039',
        entityType: 'incident',
        currentSyncState: 'PENDING_SYNC',
        retryCount: 1,
        sourceDeviceId: 'FIELD-LOCAL-01',
        createdTimestamp: new Date(Date.now() - 48 * 60 * 1000).toISOString(),
        payload: {
          id: 'INC-1039',
      ...
      },
      {
        ...
        currentSyncState: 'PENDING_SYNC',
        retryCount: 0,
        sourceDeviceId: 'FIELD-317',
        createdTimestamp: new Date(Date.now() - 32 * 60 * 1000).toISOString(),
        payload: {
          id: 'ALR-OP-104',
      ...
      {
        localId: 'OUT-INC-1028',
        entityType: 'incident',
        currentSyncState: 'BLUETOOTH_RELAYED',
        retryCount: 0,
        sourceDeviceId: 'FIELD-LOCAL-01',
        createdTimestamp: new Date(Date.now() - 95 * 60 * 1000).toISOString(),
        payload: {
          id: 'INC-1028',
          type: 'Vehicle Breakdown',
          location: 'Corridor Delta • Sector B',
          severity: 'Medium',
          reportedBy: 'Officer V. Rawat (Field Unit 4)',
          description: 'Flatbed breakdown on shoulder. Assisted by Patrol Unit 2.',
        },
      },
      ...Array.from({ length: 8 }).map((_, idx) => ({
        localId: `OUT-SYNC-100${idx + 1}`,
        entityType: 'incident' as const,
        currentSyncState: 'SYNCED' as const,
        retryCount: 0,
        sourceDeviceId: 'FIELD-LOCAL-01',
        createdTimestamp: new Date(Date.now() - (120 + idx * 45) * 60 * 1000).toISOString(),
        payload: {
          id: `INC-100${idx + 1}`,
          type: 'Route Clearance Verification',
          location: `Corridor Delta • Sector ${String.fromCharCode(65 + idx)}`,
          severity: 'Low',
          reportedBy: 'Officer V. Rawat (Field Unit 4)',
        },
      })),
    ...
    for (const r of seedRecords) {
      await queueOutboxRecord(r);
    }
  }

  const existingProcessed = await getProcessedMessages();
  if (existingProcessed.length === 0) {
    const seedProcessed: ProcessedMessageRecord[] = Array.from({ length: 12 }).map((_, idx) => ({
      messageId: `BLE-MSG-10${idx + 30}`,
      messageType: idx % 3 === 0 ? 'OPERATIONAL_ALERT' : 'INCIDENT_REPORT',
      receivedAt: new Date(Date.now() - (idx * 22 + 10) * 60 * 1000).toISOString(),
      sourceDeviceId: idx % 2 === 0 ? 'FIELD-204' : 'FIELD-317',
      sourceUserId: 'Unit 2 Patrol Operator',
      hopCount: (idx % 2) + 1,
    }));
    for (const p of seedProcessed) {
      await saveProcessedMessage(p);
    }
  }
}

...
import { isBluetoothModeActive, isRelayModeActive } from './connectivitySettings';
...
export async function processIncomingEnvelope(
  ...
  if (envelope.messageType === 'INCIDENT_REPORT') {
    ...
    // Only forward for other field devices if Relay Mode is active!
    if (isRelayModeActive() && envelope.hopCount + 1 < envelope.maxHops) {
  ...
  if (envelope.messageType === 'OPERATIONAL_ALERT') {
    ...
    // Queue in Outbox as PENDING_SYNC for store-and-forward if within hop limit and Relay Mode active
    if (isRelayModeActive() && envelope.hopCount + 1 < envelope.maxHops) {
      await queueOutboxRecord({
...
export async function relayPendingReportsToBluetooth(): Promise<RelayResult> {
  if (!isBluetoothModeActive()) {
    return {
      ...
      relayedCount: 0,
      failedCount: 0,
      details: 'Bluetooth Mode is turned OFF in Connectivity settings.',
    };
  }
  ...
  if (!isRelayModeActive()) {
    return {
      ...
      relayedCount: 0,
      failedCount: 0,
      details: 'Relay Mode is turned OFF in Connectivity settings.',
    };
  }

    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between gap-3 border-b border-neutral-200/90 pb-3">
        <div className="min-w-0 flex-1">
          <h1 className="text-base font-bold text-neutral-900">{t.myReportsHeaderTitle}</h1>
          <p className="truncate text-xs text-neutral-500">
            Filed by Driver V. Rawat (Unit 4) • Corridor Delta
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-1 text-xs -mx-1 px-1">
        ...
        {counts.pendingSync > 0 && (
          ...
          >
            ...
            <span>{t.statusPendingSync} ({counts.pendingSync})</span>
          </button>
        )}
        <button
          type="button"
        ...
        >
          {t.statusReported} ({counts.reported})
        </button>
        <button
          type="button"
        ...
        >
          {t.statusUnderReview} ({counts.underReview})
        </button>
        <button
          type="button"
      ...
      {filteredIncidents.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-neutral-300 bg-white p-8 text-center text-neutral-500 shadow-xs">
          ...
          <p className="text-sm font-semibold text-neutral-800">{t.noReportsFound}</p>
                ...
                <div className="flex items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    <FieldBadge variant={severityVar as FieldBadgeVariant}>
                      {incident.severity === 'critical' ? t.sevCritical : incident.severity === 'high' ? t.sevHigh : incident.severity === 'moderate' ? t.sevMedium : t.sevLow}
                    </FieldBadge>
                    <FieldBadge variant={statusVar as FieldBadgeVariant}>
                      {incident.status === 'Reported' ? t.statusReported : incident.status === 'Under Review' ? t.statusUnderReview : t.statusResolved}
                    </FieldBadge>
                  </div>
                </div>

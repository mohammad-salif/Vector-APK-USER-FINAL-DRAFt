...
export function FieldReportIncidentView({
  ...
  const severityOptions: Array<{ value: Severity; label: string; description: string; selectedColor: string }> = [
    { value: 'low', label: t.sevLow, description: t.sevLowDesc, selectedColor: 'border-emerald-300 bg-emerald-50 text-emerald-800 ring-2 ring-emerald-400/50' },
    { value: 'moderate', label: t.sevMedium, description: t.sevMediumDesc, selectedColor: 'border-amber-300 bg-amber-50 text-amber-800 ring-2 ring-amber-400/50' },
    { value: 'high', label: t.sevHigh, description: t.sevHighDesc, selectedColor: 'border-orange-300 bg-orange-50 text-orange-800 ring-2 ring-orange-400/50' },
    { value: 'critical', label: t.sevCritical, description: t.sevCriticalDesc, selectedColor: 'border-red-300 bg-red-50 text-red-800 ring-2 ring-red-400/50' },
  ];
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!type) {
      setFormError(t.selectDisruptionTypeError);
      return;
    }
    if (!location.trim()) {
      setFormError(t.specifyLocationError);
      return;
    }
    if (!description.trim()) {
      setFormError(t.provideDescriptionError);
      return;
    }
    ...
    try {
      const newIncident = await reportIncident({
        ...
        reportedBy: 'Driver V. Rawat (Field Unit 4)',
      });

      setIsSubmitting(false);
      onSubmitted(newIncident);
    } catch {
      setFormError(t.failedToRecordIncident);
      setIsSubmitting(false);
    }
  }
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center gap-3 border-b border-neutral-200/90 pb-3">
        <button
          type="button"
          onClick={onBack}
          aria-label={t.backToFieldPortal}
        ...
        <div className="min-w-0 flex-1">
          <h1 className="truncate text-base font-bold text-neutral-900">{t.reportRoadDisruption}</h1>
          <p className="text-xs text-neutral-500">{t.officialIncidentSubmission}</p>
        </div>
      </div>
      ...
      >
        <div className="flex items-start gap-2">
          ...
          <div className="min-w-0 flex-1 text-[11px] leading-relaxed">
            {isOnline ? (
              <p>
                <strong className="text-neutral-950 font-bold">{t.offlineReadyNoticeTitle}:</strong> {t.offlineReadyNoticeDesc}
              </p>
            ) : (
              <p>
                <strong className="text-amber-950 font-bold">{t.operatingOfflineNoticeTitle}:</strong> {t.operatingOfflineNoticeDesc}
              </p>
            )}
          </div>
        </div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4 sm:gap-5">
        ...
        <div className="space-y-2">
          <label className="text-xs font-semibold uppercase tracking-wider text-neutral-800">
            {t.disruptionTypeLabel} <span className="text-red-600">*</span>
          </label>
              ...
              return (
                ...
                >
                  ...
                  <span className={`mt-1 text-[10px] ${isSelected ? 'text-neutral-300' : 'text-neutral-400'}`}>
                    {isSelected ? t.selectedType : t.tapToSelect}
                  </span>
                </button>
              );
            })}
        ...
        <div className="space-y-1.5">
          <label className="text-xs font-semibold uppercase tracking-wider text-neutral-800">
            {t.affectedCorridorRoute} <span className="text-[11px] font-normal text-neutral-500">({t.optionalLabel})</span>
          </label>
          <div className="relative">
            ...
            >
              <option value="">{t.selectKnownCorridor}</option>
        ...
        <div className="space-y-2.5 rounded-xl border border-neutral-200/90 bg-white p-3 sm:p-3.5 shadow-xs">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <label className="text-xs font-semibold uppercase tracking-wider text-neutral-800">
              {t.locationCoordinates} <span className="text-red-600">*</span>
            </label>
            <button
              type="button"
            ...
            >
              ...
              {isLocating ? t.acquiringGps : t.useCurrentGps}
            </button>
          </div>
          ...
          {gpsCoordinates && (
            <div className="flex flex-col gap-1.5 rounded-lg border border-emerald-200 bg-emerald-50 p-2.5 text-xs text-emerald-800 sm:flex-row sm:items-center sm:justify-between sm:px-3 sm:py-2">
              ...
              >
                {showCoordinatesEditor ? t.hideDetailsAction : t.viewDetailsAction}
              </button>
            </div>
          )}
        ...
        <div className="space-y-2">
          <label className="text-xs font-semibold uppercase tracking-wider text-neutral-800">
            {t.severityLevelLabel} <span className="text-red-600">*</span>
          </label>
        ...
        <div className="space-y-1.5">
          <label className="text-xs font-semibold uppercase tracking-wider text-neutral-800">
            {t.fieldObservationsDesc} <span className="text-red-600">*</span>
          </label>
          <textarea
            value={description}
        ...
        <div className="space-y-2 rounded-xl border border-neutral-200/90 bg-white p-3 sm:p-3.5 shadow-xs">
          <div className="flex items-center justify-between">
            <label className="text-xs font-semibold uppercase tracking-wider text-neutral-800">
              {t.fieldPhotograph} <span className="text-[11px] font-normal text-neutral-500">({t.optionalLabel})</span>
            </label>
          ...
          >
            {isSubmitting ? (
              ...
              <>
                ...
                <span>{t.submitIncidentReport}</span>
              </>
            )}
          </button>
          <button

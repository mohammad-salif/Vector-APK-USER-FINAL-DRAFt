...
export function FieldRouteView({
  ...
  const riskLevels = [
    { key: 'low', label: t.sevLow, range: '0-30', active: route.riskLevel === 'low' },
    { key: 'moderate', label: t.sevMedium, range: '31-65', active: route.riskLevel === 'moderate' },
    { key: 'high', label: t.sevHigh, range: '66-85', active: route.riskLevel === 'high' },
    { key: 'critical', label: t.sevCritical, range: '86-100', active: route.riskLevel === 'critical' },
  ];
        ...
        <div className="mt-3 grid grid-cols-1 gap-2.5 sm:grid-cols-2 text-xs">
          ...
          <div className="rounded-lg border border-neutral-200 bg-neutral-50/80 p-3">
            ...
            <div className="mt-2 flex items-center gap-2 border-t border-neutral-200/80 pt-2 text-[10px] font-mono text-neutral-500">
              <span>{t.assignedVehicle}: <strong>{vehicle.id}</strong></span>
              <span>•</span>
              <span>13.5 mi ahead</span>
            </div>
          </div>
          ...
          <div className="rounded-lg border border-neutral-200 bg-neutral-50/80 p-3">
            ...
            <div className="mt-2 flex items-center gap-2 border-t border-neutral-200/80 pt-2 text-[10px] text-neutral-500">
              <span className="font-medium text-red-700">{t.statusBlocked}: Mile 42</span>
            </div>
          </div>
        </div>
      </div>
      ...
      >
          ...
          <div>
            ...
            <div className="mt-1 flex items-baseline gap-2">
              ...
              <FieldBadge variant="critical">
                {t.sevCritical} ({route.riskScore})
              </FieldBadge>
            </div>
          </div>

          <div className="flex flex-col items-end">
            <span className="text-[10px] uppercase font-semibold text-neutral-400">{t.corridorStatusLabel}</span>
        ...
        <div className="mt-3.5">
          <div className="flex items-center justify-between text-[11px] text-neutral-500 mb-1.5 font-medium">
            <span>{t.riskIndexLabel} Spectrum</span>
            <span className="font-bold text-red-700">{t.sevCritical} (86-100)</span>
          </div>
        ...
        <div className="mt-3 flex items-center gap-2 rounded-lg border border-neutral-200 bg-neutral-50/80 px-3 py-2 text-xs text-neutral-700">
          ...
          <span>
            <strong>{t.travelImpactLabel}:</strong> Indefinite stoppage on Corridor Delta. Heavy equipment clearance required before reopening.
          </span>
        </div>
      </div>
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-baseline justify-between gap-1">
                <strong className="text-red-900 font-bold">
                  {t.statusBlocked} (Mile 42 — Incident INC-3401)
                </strong>
                <span className="text-[10px] font-mono text-red-700 font-bold shrink-0">{t.sevCritical}</span>
              </div>
              <p className="mt-0.5 leading-relaxed text-red-800">
        ...
        <div className="mt-3.5 pt-3 border-t border-neutral-100">
          <div className="flex items-center justify-between mb-2">
            <span className="text-[11px] font-bold text-neutral-800 flex items-center gap-1.5">
              ...
              {t.verifiedIncidentsTitle} ({incidentsOnRoute.length})
            </span>
            <span className="text-[10px] text-neutral-400">{t.clickForDetails}</span>
          </div>

          <div className="space-y-2">
            {incidentsOnRoute.length === 0 ? (
              <p className="text-xs text-neutral-500 py-2 text-center">{t.noIncidentsOnRoute}</p>
            ) : (
          ...
          <div>
            <div className="flex flex-wrap items-baseline justify-between gap-1">
              ...
              <span className="font-bold text-emerald-700">
                {t.sevLow} ({t.riskIndexLabel} {alternativeRoute.riskScore} / 100)
              </span>
            </div>
            ...
            <div className="grid grid-cols-2 divide-x divide-neutral-200 text-xs">
              ...
              <div className="p-3 bg-red-50/40">
                <div className="flex items-center justify-between pb-1.5 border-b border-red-100">
                  <span className="font-mono font-bold text-red-900">{t.currentRouteLabel}: {route.id}</span>
                  <span className="rounded bg-red-100 px-1.5 py-0.5 text-[10px] font-bold text-red-800">{t.statusBlocked}</span>
                </div>
                <div className="mt-2 space-y-1.5 text-[11px]">
                  <div>
                    <span className="text-[10px] uppercase text-neutral-400 block font-medium">{t.riskIndexLabel}</span>
                    <strong className="font-mono text-red-700 text-xs">{route.riskScore} / 100 ({t.statusCritical})</strong>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <span className="text-red-800 font-medium">{t.statusBlocked}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="p-3 bg-emerald-50/40">
                <div className="flex items-center justify-between pb-1.5 border-b border-emerald-100">
                  <span className="font-mono font-bold text-emerald-900">{t.alternateRouteLabel}: {alternativeRoute.id}</span>
                  <span className="rounded bg-emerald-100 px-1.5 py-0.5 text-[10px] font-bold text-emerald-800">{t.statusOpen}</span>
                </div>
                <div className="mt-2 space-y-1.5 text-[11px]">
                  <div>
                    <span className="text-[10px] uppercase text-neutral-400 block font-medium">{t.riskIndexLabel}</span>
                    <strong className="font-mono text-emerald-700 text-xs">{alternativeRoute.riskScore} / 100 ({t.sevLow})</strong>
                  </div>
                  <div>
                    <span className="text-[10px] uppercase text-neutral-400 block font-medium">Distance</span>
                  ...
                  <div>
                    <span className="text-[10px] uppercase text-neutral-400 block font-medium">Condition</span>
                    <span className="text-emerald-800 font-medium">{t.statusAccessible}</span>
                  </div>
                </div>
              </div>
            </div>

...
import { FieldLanguageSelectView } from '@/components/field/FieldLanguageSelectView';
import {
  type AppLanguage,
  isLanguageOnboarded,
...
export function FieldPortalPage() {
  const { lang, setLanguage } = useI18n();
  // First-Launch Language Selection State
  const [hasCompletedLanguage, setHasCompletedLanguage] = useState<boolean>(() =>
    isLanguageOnboarded()
  );

  // Authentication State (App Launch -> Language -> Login -> Authenticated Driver -> Home)
  ...
  function handleLoginSuccess(_accountOrId?: unknown) {
  ...
  // 1. FIRST-LAUNCH FLOW: Choose your language (unified searchable list)
  ...
  // 2. AUTHENTICATION FLOW: Driver Field Login & Registration
  if (!isAuthenticated) {
